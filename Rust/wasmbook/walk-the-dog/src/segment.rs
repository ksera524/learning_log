...
use crate::{engine::{Image, Point, Rect, SpriteSheet}, game::{Obstacle, Platform,Barrier}};
...
const FLOATING_PLATFORM_BOUNDING_BOXES: [Rect; 3] = [
    Rect {position:Point{ x: 0, y: 0}, width: PLATFORM_EDGE_WIDTH, height: PLATFORM_EDGE_HEIGHT },
    Rect {position: Point { x: PLATFORM_EDGE_WIDTH, y: 0}, width: PLATFORM_WIDTH - (PLATFORM_EDGE_WIDTH * 2), height: PLATFORM_HEIGHT },
    Rect {position: Point{ x: PLATFORM_WIDTH - PLATFORM_EDGE_WIDTH, y: 0}, width: PLATFORM_EDGE_WIDTH, height: PLATFORM_EDGE_HEIGHT },
];
