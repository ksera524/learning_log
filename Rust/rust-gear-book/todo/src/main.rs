use actix_web::{get, web::{self, Data}, App, HttpResponse, HttpServer, ResponseError,http::header,post};
use askama::Template;
use r2d2::Pool;
...
#[post("/add")]
async fn add_todo(params: web::Form<AddParams>,db:Data<r2d2::Pool<SqliteConnectionManager>>) -> Result<HttpResponse,MyError>{
    let conn = db.get()?;
...
#[post("/delete")]
async fn delete_todo(params: web::Form<DeleteParams>,db:Data<r2d2::Pool<SqliteConnectionManager>>) -> Result<HttpResponse,MyError>{
    let conn =db.get()?;
