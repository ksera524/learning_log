...
pub fn recommended_books(friend: String) -> Vec<Book> {
    let scala = vec![
        Book::new(
            "FP in Scala".to_string(),
            vec!["Chiusano".to_string(), "Bjarnason".to_string()],
        ),
        Book::new(
            "Get Programing with Scala".to_string(),
            vec!["Sfregola".to_string()],
        ),
    ];
        ...
        .flat_map(|book| {
            book.authors.iter().flat_map(move |author| {
                book_adaptations(author).into_iter().map(move |movie| {
                    format!(
                        "You may like {} because you like {}'s {}",
